...
H~0~: **pH does not predict abolone growth (change in shell surface area per daymm^2^ d^-1^). **  
...
# base layer of gg plot call
ggplot(data = abalone_clean, # data set clean
       aes(x = mean_ph, # x axis is predictor, y is response
           y = change_in_area)) +
  # first layer points 
  geom_point(size = 4,
             stroke = 1,
...
**Tank B1B where pH censor failed**  
...
```{r abalone-model}
abalone_model <- lm(
  change_in_area ~ mean_ph, # change in shell surface area per day as a function of pH 
  data = abalone_clean      # 
)
```
...
par(mfrow = c(2, 2))   # display residual plots
plot(abalone_model)    # plot them 

```

Redisuals seem homosecadatic (consistant across values) based on residuals vs fitted and scale location plots. Residuals also seem normally disturbuted based on QQ plot. No major outliars based on residuals vs leverage plot. 

**Written communication** We evaluated model assumptions visually. For homesecdacituy of residential we used residuals vs fitted ans scale location plots. For normality of residuals we used QQ plots. Based on our diagnosis, thjere were no major deviations from homoscedasticity or normality of residiuals. There were also no out liars that might influence model estimates or predictions. 
...
**2.98 +- 0.76 SE. With each increase in pH, the model predicts an increase of 2.98 +- 0.76 SE in change in shell surface area per day **  

What is the intercept estimate (with standard error?)  

**-18.5 +- 6.16 SE. When pH= 0 the model predicts that abalone change in shell surface area per day is -18.5 +- 6.16 (note: thi sis not necessarily biologically realiztc but something the model generates as an estimate **
...
**conflow and confhigh: range for the 95% CI around moel predictions**
...
**ggeffects output object and data frame **  
...
**at ph 8 the model predicts that abalone change in shell surface area per day should be 5.36 (95% CI: [5.08, 5.64])**
